...
const QR_INFO_VALIDATOR_URL = z.object({
  url: z.string().url(),
}).or(z.object({
  relativeUrl: z.string(),
}));

const QR_INFO_VALIDATOR = z.object({
...
type QrInfo = z.infer<typeof QR_INFO_VALIDATOR>;
...
router.getRaw("/", (req, res) => {
  const queryCheck = QR_INFO_VALIDATOR.safeParse(req.query);
...
router.postRaw("/", express.json(), (req, res) => {
  const bodyCheck = QR_INFO_VALIDATOR.safeParse(req.body);
    ...
    res
      ...
      .send(
        qrCode
          ...
          .replace(/viewBox="(?<coords>.*?)"/i, `viewBox="0 0 ${ x } ${ y + 1 + yOffset }"`)
        ,
      );
  } catch (e) {
    res.sendStatus(404);
...
const parseUrlOrRelative = (urlOrRelative: z.infer<typeof QR_INFO_VALIDATOR_URL>) => {

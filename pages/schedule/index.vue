      ...
      <div v-if="events.length > 0" :class="$style.calendarContainer">
        ...
        >
          <template #event="{ event }">
            ...
            >
              <strong :class="$style.eventTitle" v-text="event.title" />
              <br>
              <span :class="$style.eventTime">
                <time :datetime="event.start.toISOString()" v-text="formatEventTime(event.start)" />
                &ndash;
                <time :datetime="event.end.toISOString()" v-text="formatEventTime(event.end)" />
              </span>
              <div v-if="event.location" :class="$style.eventLocation">
                <i class="pi pi-map-marker" />
                {{ event.location }}
              </div>
              <div v-if="event.text" :class="$style.eventText" v-text="event.text" />
            </component>
          </template>
        </vue-calendar>
        <template v-else>
